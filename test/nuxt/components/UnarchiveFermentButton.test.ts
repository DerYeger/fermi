...
import { BASE_COMPLETED_FERMENT } from "../../data";
...
describe("components/UnarchiveFermentButton", () => {
	it("renders restore button with unarchive icon and correct props", async () => {
	...
	it("shows Restore label when hideLabel is false and hides when true", async () => {
		const wrapper = await mountSuspended(UnarchiveFermentButton, {
			props: { ferment: BASE_COMPLETED_FERMENT, hideLabel: false }
		});
		const button = wrapper.findComponent({ name: "UButton" });
		expect(button.props("label")).toBe("Restore");

		const hiddenWrapper = await mountSuspended(UnarchiveFermentButton, {
			props: { ferment: BASE_COMPLETED_FERMENT, hideLabel: true }
		});
	...
	it("opens confirmation dialog, confirms restoration, and calls update", async () => {
		...
		expect(mockUpdate).toHaveBeenCalledWith(BASE_COMPLETED_FERMENT.id, expect.any(Function));
			...
			expect.objectContaining({
				...
				actions: expect.arrayContaining([
					expect.objectContaining({
						label: "View",
						variant: "subtle",
						to: `/ferments/${BASE_COMPLETED_FERMENT.id}`
					})
				])
			})
		);
